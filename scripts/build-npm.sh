...
for target in "${targets[@]}"; do
  ...
  src="$dist/hitmaker-$goos-$goarch$ext"
  if [[ ! -f "$src" ]]; then
    echo "missing $src — run 'make release-build' first" >&2
    exit 1
  fi

  name="hitmaker-$node_platform-$node_arch"
  pkg="$packages/$name"
  mkdir -p "$pkg/bin"

  cp "$src" "$pkg/bin/hitmaker$ext"
  chmod +x "$pkg/bin/hitmaker$ext"
  ...
  cat > "$pkg/package.json" <<JSON
{
  "name": "@zeb-link/$name",
  "version": "$version",
  "description": "Native hitmaker binary for $node_platform $node_arch. Installed automatically by hitmaker.",
  "repository": {
    "type": "git",
  ...
  "os": ["$node_platform"],
  "cpu": ["$node_arch"],
  "files": ["bin/hitmaker$ext", "LICENSE"]
}
JSON
...
The native \`hitmaker\` binary for $node_platform $node_arch.

You don't install this directly. It is an optional dependency of
[\`hitmaker\`](https://www.npmjs.com/package/hitmaker), which npm resolves to the
one package matching your platform.
