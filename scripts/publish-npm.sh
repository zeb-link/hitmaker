...
#
# Normally you do not run this by hand — pushing a v* tag makes GitHub Actions
# publish via trusted publishing (OIDC), which needs no token and attaches
# provenance. See RELEASE.md. This script is the local fallback, and the way the
# first release of a brand-new package gets created (npm can't configure trusted
# publishing for a package that doesn't exist yet).
#
# Dry run (default):  scripts/publish-npm.sh
# For real:           PUBLISH=1 scripts/publish-npm.sh
#
# A local publish needs one of:
#   - a granular token with "bypass 2FA" in ~/.npmrc (being deprecated by npm)
...
if [[ "$publish" == "1" ]]; then
  echo "published v$version — smoke test with: npm i -g hitmaker && hitmaker version"
else
